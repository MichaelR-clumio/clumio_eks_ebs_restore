...
class EC2BackupList(API):
    ...
    def set_search_time_frame_before_after(self, search_type):
        self.search_type = search_type
        if self.search_type == "before":
            self.filter_expression = "$gt"
            self.start_hour = "00"
            self.start_min = "00"
            self.start_sec = "00"
        elif self.search_type == "after":
            self.filter_expression = "$lte"
            self.start_hour = "23"
            self.start_min = "59"
            self.start_sec = "59"
...
class OnDemandBackupEC2(API):
    ...
    def ec2_backup_from_file(self, filename, bucket, prefix, role, aws_session, region):
        if self.setup_import_file_s3(filename, bucket, prefix, role, aws_session, region="us-east-2"):
            if self.data_import():
                # print(f"len of record {len(self.import_data.get("records",[]))}")
                for record in self.import_data.get("records", []):
                    # print(f"record to restore: {record}")
                    run_results = self.run_backup_record(record)
                    # print(run_results)
                    # print(f"restore results for {record.get("instanceId",None)} {run_results}")
                    if self.log_activity_flag:
                        pass
...
class DynamoDBBackupList(API):
    ...
    def set_search_time_frame_before_after(self, search_type):
        # External function - must be called to set search type

        good_types = ["before", "after"]
        if not search_type in good_types:
            return False
        self.search_type = search_type
        if self.search_type == "before":
            self.filter_expression = "$gt"
